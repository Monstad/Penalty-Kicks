...
SCENARIO_EV_AFTER_FAKE = {      # Defining the scenario payoffs for the penalty taker after choosing 'fake the shot'.
    'commit_left': 1,
    'lean_left': 1,
    'stay_middle': 0.6,
    'lean_right': 1,
...
def getScenarioData(iterations, aim_options, area_options, existing_data=None, replace=True):
    ...
    for i in range(len(aim_options)):
        for j in range(len(area_options)):
            if replace and aim_options[i] != 'fake_the_shot':
...
def makeScenariosSymmetric(scenario_data, aim_options, area_options):
    for i in range(len(aim_options)):
        for j in range(len(area_options)):
            if aim_options[i] != 'fake_the_shot':
                x = aim_options[i][0][0]
                y = aim_options[i][0][1]
...
def getStrategyEVs(scenario_data, aim_options, area_options, strategy_pt, strategy_gk):
    ...
    for i in range(len(aim_options)):
        for j in range(len(area_options)):
            exp_value_pt += scenario_data[(aim_options[i], area_options[j])] * strategy_pt[i] * strategy_gk[j]
    exp_value_gk = -exp_value_pt
...
def getExploitability(scenario_data, aim_options, area_options, strategy_pt, strategy_gk):
    num_options_pt = len(aim_options)
    num_options_gk = len(area_options)
    exp_value_pt, exp_value_gk = getStrategyEVs(scenario_data, aim_options, area_options, strategy_pt, strategy_gk)
    ...
    for i in range(num_options_pt):
        ...
        option_ev = getStrategyEVs(scenario_data, aim_options, area_options, pure_strategy_pt, strategy_gk)[0]
    ...
    for i in range(num_options_gk):
        ...
        option_ev = getStrategyEVs(scenario_data, aim_options, area_options, strategy_pt, pure_strategy_gk)[1]
...
def getNewAimOptions(aim_option, distance, aim_options):
    ...
    new_aim_options = ['fake_the_shot']
    if aim_option != 'fake_the_shot':
...
scenarioData['fake_the_shot', 'commit_left'] = 1
scenarioData['fake_the_shot', 'lean_left'] = 1
scenarioData['fake_the_shot', 'stay_middle'] = 0.6
scenarioData['fake_the_shot', 'lean_right'] = 1
scenarioData['fake_the_shot', 'commit_right'] = 1

firstIteration = True
INITIAL_AIM_OPTIONS.append('fake_the_shot')
...
while True:
    ...
    for option in viableOptions:
        ...
        if (option in frequentAimOptions) and (option != 'fake_the_shot') and isSurrounded and distance == SPECIFICITY:
            num_converged += 1
