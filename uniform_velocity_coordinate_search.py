...
STD_DEV = 2                         # Defining the aiming inaccuracy.
VELOCITY = 10                       # Defining the velocity factor. (should be between 0 and 10)
...
def getScenarioEV(iterations, aim_coordinate, area, velocity, std_dev):
    ...
    for _ in range(iterations):
        hit_coordinate, hit_velocity = getHitCoordinate(aim_coordinate, std_dev, velocity)
        if isInsideScoringArea(hit_coordinate) and not isSavedShot(hit_coordinate, hit_velocity, area):
            goal += 1
    return goal/iterations


def getScenarioData(iterations, aim_options, area_options, velocity, std_dev, existing_data=None, replace=True):
    ...
    for i in range(len(aim_options)):
        for j in range(len(area_options)):
            if replace:
                scenario_ev = getScenarioEV(iterations, aim_options[i], area_options[j], velocity, std_dev)
    ...
    for i in range(len(aim_options)):
        for j in range(len(area_options)):
            ...
            if x <= 12:
                ...
                symmetrical_EV = scenario_data[((symmetrical_x, symmetrical_y), symmetrical_area)]
                EV = scenario_data[((x, y), area_options[j])]
                avg_EV = (EV + symmetrical_EV) / 2
                scenario_data[((x, y), area_options[j])] = avg_EV
                scenario_data[((symmetrical_x, symmetrical_y), symmetrical_area)] = avg_EV
    return scenario_data
...
def makeStrategySymmetricPT(strategy, aim_options):
    ...
    for i in range(len(aim_options)):
        ...
        if x == 12:
            ...
            symmetrical_index = aim_options.index((symmetrical_x, y))
...
firstIteration = True
scenarioData = getScenarioData(FIRST_SCENARIO_ITERATIONS, INITIAL_AIM_OPTIONS, AREA_OPTIONS, VELOCITY, STD_DEV)
...
while True:
    ...
    if firstIteration:
        scenarioData = getScenarioData(NEXT_SCENARIO_ITERATIONS, allNewAimOptions, AREA_OPTIONS, VELOCITY, STD_DEV, existing_data=scenarioData, replace=True)
        firstIteration = False
    else:
        scenarioData = getScenarioData(NEXT_SCENARIO_ITERATIONS, allNewAimOptions, AREA_OPTIONS, VELOCITY, STD_DEV, existing_data=scenarioData, replace=False)


scenarioData = getScenarioData(FINAL_SCENARIO_ITERATIONS, frequentAimOptions, AREA_OPTIONS, VELOCITY, STD_DEV, existing_data=scenarioData, replace=True)
