...
def getStrategyEVs(scenario_data, aim_options, area_options, strategy_pt, strategy_gk):
    ...
    for i in range(len(aim_options)):
        for j in range(len(area_options)):
            key = (aim_options[i], area_options[j])
            exp_value_pt += \
                scenario_data[key] * strategy_pt[i] * strategy_gk[j]
    exp_value_gk = -exp_value_pt
...
for area_option, scenario_ev in SCENARIO_EV_AFTER_FAKE.items():
    scenarioData[('fake_the_shot', area_option)] = scenario_ev
AIM_OPTIONS.append('fake_the_shot')
